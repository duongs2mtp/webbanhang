...
public class UpdatePhieuXuat extends javax.swing.JDialog {
        ...
        try {
            ...
            for (var i : arrProd) {
                tblModel.addRow(new Object[]{
                    i.getmaSanPham(), i.gettenSanPham(), i.getsoLuong(), formatter.format(i.getGia()) + "đ"
                });
            }
        } catch (Exception e) {
        }
    ...
    public SanPham findSanPham(String maSanPham) {
        for (var i : allProduct) {
            if (maSanPham.equals(i.getmaSanPham())) {
                return i;
            }
        }
        return null;
    ...
    public ChiTietPhieu findCTPhieu(String maSanPham) {
        for (var i : CTPhieu) {
            if (maSanPham.equals(i.getMaSanPham())) {
                return i;
            }
        ...
        try {
            ...
            for (int i = 0; i < CTPhieu.size(); i++) {
                tblNhapHangmd.addRow(new Object[]{
                    i + 1, CTPhieu.get(i).getMaSanPham(), findSanPham(CTPhieu.get(i).getMaSanPham()).gettenSanPham(), CTPhieu.get(i).getSoLuong(), formatter.format(CTPhieu.get(i).getDonGia()) + "đ"
                });
            }
        } catch (Exception e) {
        }
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        if (CTPhieu.isEmpty()) {
            ...
            for (var ct : CTPhieuOld) {
                SanPhamDAO.getInstance().updateSoLuong(ct.getMaSanPham(), SanPhamDAO.getInstance().selectById(ct.getMaSanPham()).getsoLuong() + ct.getSoLuong());
        ...
        if (i_row == -1) {
            ...
            if (newSL != null) {
                ...
                try {
                    ...
                    if (soLuong > findSanPham(CTPhieu.get(i_row).getMaSanPham()).getsoLuong()) {
            ...
            if (soluongselect == 0) {
                ...
                if (soluongselect < soluong) {
                    ...
                    if (mtl != null) {
                        ...
                        ChiTietPhieu ctp = new ChiTietPhieu(phieuxuat.getMaPhieu(), mt.getmaSanPham(), soluong, mt.getGia());
                        CTPhieu.add(ctp);
                    }
                    loadDataToTableNhapHang();
                    textTongTien.setText(formatter.format(tinhTongTien()) + "đ");
