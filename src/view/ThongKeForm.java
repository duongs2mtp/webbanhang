...
public class ThongKeForm extends javax.swing.JInternalFrame {
    ...
    public ThongKeForm() {
        ...
        txtQuantityProduct.setText(Integer.toString(SanPhamDAO.getInstance().getSl()));
    ...
    private void loadDataToTableThongKeProduct(ArrayList<ThongKeProduct> thongKe) {
        try {
            ...
            for (int i = 0; i < thongKe.size(); i++) {
                tblModelAcc.addRow(new Object[]{
                    (i + 1), thongKe.get(i).getMaSanPham(), thongKe.get(i).getTenSanPham(), thongKe.get(i).getSlNhap(), thongKe.get(i).getSlXuat()
                });
            }
    ...
    private ArrayList<ThongKeProduct> searchTenSanPhamThongKe(ArrayList<ThongKeProduct> arr, String name) {
        ...
        for (ThongKeProduct i : arr) {
            if (i.getMaSanPham().toLowerCase().contains(name.toLowerCase()) || i.getTenSanPham().toLowerCase().contains(name.toLowerCase())) {
                result.add(i);
            }
        }
        return result;
