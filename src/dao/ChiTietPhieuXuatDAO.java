...
public class ChiTietPhieuXuatDAO implements DAOInterface<ChiTietPhieu> {
    ...
    public int insert(ChiTietPhieu t) {
        ...
        try {
            ...
            String sql = "INSERT INTO ChiTietPhieuXuat (maPhieu, maSanPham, soLuong, donGia) VALUES (?,?,?,?)";
    ...
    public int update(ChiTietPhieu t) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "UPDATE ChiTietPhieuXuat SET maPhieu=?, maSanPham=?, soLuong=?, donGia = ?  WHERE maPhieu=? AND maSanPham=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            pst.setString(2, t.getMaSanPham());
            pst.setInt(3, t.getSoLuong());
    ...
    public ArrayList<ChiTietPhieu> selectAll(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ChiTietPhieu ctp = new ChiTietPhieu(maPhieu, maSanPham, soLuong, donGia);
                ketQua.add(ctp);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
    ...
    public ChiTietPhieu selectById(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int soLuong = rs.getInt("soLuong");
                double donGia = rs.getDouble("donGia");
                ketQua = new ChiTietPhieu(maPhieu, maSanPham, soLuong, donGia);
            }
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
